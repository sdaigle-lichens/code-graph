    ...
    constructor(config = {}) {
        const { url = "http://127.0.0.1:8529", auth, arangoVersion = 31100, loadBalancingStrategy = "NONE", maxRetries = 0, poolSize = 3 *
            (loadBalancingStrategy === "ROUND_ROBIN" && Array.isArray(url)
                ? url.length
                : 1), fetchOptions: { headers, ...commonFetchOptions } = {}, agentOptions, onError, precaptureStackTraces = false, responseQueueTimeSamples = 10, ...commonRequestOptions } = config;
        ...
        this._commonFetchOptions.headers.set("x-arango-driver", `arangojs/10.3.0 (cloud)`);
